...
import {
  ...
  MsgVpnClientProfileCreateResponse,
} from '../../../types/msgvpn-client-profile.js'

export default class BrokerClientProfileCreate extends ScBrokerCommand<typeof BrokerClientProfileCreate> {
  static override args = {}
  static override description = `Create a Client Profile on a Solace Event Broker.
...
Any attribute missing from the request will be set to its default value. The creation of instances of this object are synchronized to HA mates and replication sites via config-sync.`
  ...
  public async run(): Promise<MsgVpnClientProfileCreateResponse> {
    const {flags} = await this.parse(BrokerClientProfileCreate)

    // Build client profile creation request body
    const clientProfileBody: MsgVpnClientProfileCreateRequest = this.buildClientProfileRequest(flags)

    // Make SEMP Config API call to create the client profile
    const endpoint = `/SEMP/v2/config/msgVpns/${this.msgVpnName}/clientProfiles`
    const sempResp = await this.sempConn.post<MsgVpnClientProfileCreateResponse>(endpoint, clientProfileBody)
  ...
  /**
   * Builds the client profile creation request body from command flags
   */
  private buildClientProfileRequest(flags: {
